...
#include <QRegularExpression>       //  for suit qt6
...
void MainWindow::init()
{
    ...
    QRegularExpression regx("[0-9]+$"); // 设置正则表达式：只能输入数字
    QRegularExpressionValidator *numberOnlyValidator = new QRegularExpressionValidator(regx, mTimerSendLineEdit);
    ...
    QDateTime dt = QDateTime::fromSecsSinceEpoch((uint)getDateFromMacro(__DATE__));
    this->setWindowTitle("sscom for Linux/Mac 0.4.1, github/kangear/sscom, manual port by kk");
...
//! [3]
    ...
    // connect(ui->lineEdit, SIGNAL(currentIndexChanged(int)),
...
static QString hexToQString(bool isDebug, QString hexStr) {
    ...
    // Split hexStr into a list of strings by spaces
    ...
    for (const QString& qs : list) {
        bool bStatus = false;
        int a = qs.toInt(&bStatus, 16);  // Convert hex string to integer

        if (bStatus && (qs.length() == 2)) {  // Ensure the hex value is 2 characters long
            QString sA = QString(QChar(a));   // Convert the integer to a character

            if (isDebug) {
                qDebug() << "a:" << a << "sA:" << sA << "qs:" + qs;
            }

            ret.append(sA);  // Append character to result
        } else {
            if (isDebug) {
                qDebug() << "error!!!";  // Error: Conversion failed
            }
...
void MainWindow::updateSettings()
{
    ...
    currentSettings.name = ui->lineEdit->text();
